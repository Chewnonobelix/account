...
{
    if (!m_from.isValid() || d < m_from) m_from = d;
    if (!m_to.isValid()   || d > m_to)   m_to   = d;
}
...
{
    for (const auto& t : list) addTransaction(t);
}
...
double Total::evaluate() const
{
    ...
    for (const auto& tp : m_transactions) {
        ...
        const double signedVal =
            (m == OpenAccountEnums::Movement::Credit) ? (+v) :
            (m == OpenAccountEnums::Movement::Debit)  ? (-v) : v;
        sum += signedVal;
    }
    return sum;
}
...
Total operator+(const Total& a, const Total& b)
{
    ...
    for (const auto& t : a.m_transactions) out.addTransaction(t);
    for (const auto& t : b.m_transactions) out.addTransaction(t);
    return out;
}
...
Total operator+(const Total& a, Transaction& t)
{
    ...
    Q_ASSERT_X(!sp.isNull(), "Total::operator+(Total, Transaction)",
               "Transaction must be owned by a QSharedPointer");
