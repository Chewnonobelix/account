...
private slots:
    ...
    void json_roundtrip() {
        ...
        // id is restored from JSON (Transaction ctor sets a uuid first, then overrides if present)
        QCOMPARE(r.id(), id);
    ...
    void plus_tx_tx_builds_total() {
        ...
        // Operators accept references; sharedFromThis() is valid because objects are owned by QSharedPointer
        Total T = *t1 + *t2;
    ...
    void plus_chain_accumulates_and_bounds() {
        auto t1 = QSharedPointer<Transaction>::create(); t1->setValue(10.0); t1->setMovement(OpenAccountEnums::Movement::Credit); t1->setDate(QDate(2025, 3, 1));
        auto t2 = QSharedPointer<Transaction>::create(); t2->setValue( 3.0); t2->setMovement(OpenAccountEnums::Movement::Debit ); t2->setDate(QDate(2025, 3, 4));
        auto t3 = QSharedPointer<Transaction>::create(); t3->setValue(20.0); t3->setMovement(OpenAccountEnums::Movement::Credit); t3->setDate(QDate(2025, 2, 25));
