...
void MainModel::onUpdateEntry(QUuid id)
{
    ...
    m_fw.setFuture(entries);
    connect(&m_fw, &QFutureWatcher<QMap<QUuid, Entry>>::finished, [this]() {
        qDebug()<<"Mainmodel"<<"FutureEnd";
        auto entries = m_fw.result();
        for (auto it : entries){
            insertData(it);
        }
        ...
        for(auto it = 0; it < m_displayModel.size(); it++)
            if(m_displayModel[it].e.id() == m_selected) {
                setCurrentIndex(it);
            }
    });
