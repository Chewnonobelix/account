...
void TargetListModel::setCurrentId(QString id) {
	...
	m_list = AbstractController::db()
							 ->selectBudgets()[m_currentBudgetId]
							 .targets()
							 .values();
...
QVariant TargetListModel::budget() const { return m_budget; }

void TargetListModel::setBudget(QVariant budget) {
	m_budget = budget;
	setCurrentId(m_budget.value<Budget>().id().toString());
	emit budgetChanged();
}
...
bool TargetListModel::isValid() const { return !m_currentBudgetId.isNull(); }
